...
export default function Todo({
  ...
  const [currentNameText, setCurrentNameText] = useState(name);
  ...
  const updateText = (e) => setCurrentNameText(e.target.value);
  const updatePrice = (e) => setCurrentPriceText(e.target.value);
  const editTodo = () => setEdit(true);

  const cancelEdit = () => {
    setEdit(false);
    setCurrentNameText(name);
  };

  const handleUpdateTodo = () => {
    if (currentNameText) {
      dispatch(attemptUpdateTodo(id, currentNameText)).then(() =>
        setEdit(false),
      );
    }
  };
